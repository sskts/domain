...
function authorizeMvtk(agentId, transactionId, authorizeObject) {
    ...
    return (authorizeActionRepo, transactionRepo) => __awaiter(this, void 0, void 0, function* () {
        ...
        const action = yield authorizeActionRepo.startMvtk(transaction.agent, transaction.seller, authorizeObject);
        // 特に何もしない
        // アクションを完了
...
function confirm(agentId, transactionId) {
    ...
    return (authorizeActionRepo, transactionRepo) => __awaiter(this, void 0, void 0, function* () {
        ...
        const ownershipInfos = order.acceptedOffers.map((acceptedOffer) => {
            ...
            // ひとまず十分にuniqueにしておく
            const identifier = `${acceptedOffer.itemOffered.typeOf}-${acceptedOffer.itemOffered.reservationNumber}-${now.valueOf().toString()}`;
            return factory.ownershipInfo.create({
                ...
                ownedFrom: now,
                // tslint:disable-next-line:no-suspicious-comment
                ownedThrough: moment(now).add(1, 'month').toDate(),

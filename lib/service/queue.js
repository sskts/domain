...
function executeSettleCOASeatReservationAuthorization() {
    ...
    return (assetAdapter, ownerAdapter, performanceAdapter, queueAdapter) => __awaiter(this, void 0, void 0, function* () {
        // 未実行のCOA資産移動キューを取得
        const queueDoc = yield queueAdapter.model.findOneAndUpdate({
            status: queueStatus_1.default.UNEXECUTED,
            run_at: { $lt: new Date() },
        ...
        try {
            // 失敗してもここでは戻さない(RUNNINGのまま待機)
            yield stockService.transferCOASeatReservation(queueDoc.get('authorization'))(assetAdapter, ownerAdapter, performanceAdapter);
            yield queueAdapter.model.findByIdAndUpdate(queueDoc.get('id'), { status: queueStatus_1.default.EXECUTED }, { new: true }).exec();
        }
        catch (error) {
            // 実行結果追加

...
function createFromOldTransaction(transactionId) {
    return (orderRepository, transactionRepository, filmRepository, performanceRepository, screenRepository, theaterRepository) => __awaiter(this, void 0, void 0, function* () {
        const transaction = yield transactionRepository.transactionModel.findOne({
            _id: transactionId,
            status: 'CLOSED'
        }).exec();
        if (transaction === null) {
        ...
        const detail = yield getOldTransactionDetails(transaction.id)(transactionRepository, filmRepository, performanceRepository, screenRepository, theaterRepository);
        debug('detail:', detail);
        const order = createOrder(detail);
        debug('order:', order);
        yield orderRepository.orderModel.findOneAndUpdate({
...
function getOldTransactionDetails(transactionId) {
    return (transactionRepository, filmRepository, performanceRepository, screenRepository, theaterRepository) => __awaiter(this, void 0, void 0, function* () {
        const transactionDoc = yield transactionRepository.transactionModel.findById(transactionId).populate('owners').exec();
        ...
        const authorizations = yield transactionRepository.findAuthorizationsById(transaction.id);
        // GMOオーソリを取り出す
        ...
        const performanceDoc = yield performanceRepository.model.findById(seatReservationAuthorization.assets[0].performance)
            .populate('film')
            .populate('theater')
            .populate('screen')
            .exec();
        const performance = performanceDoc.toObject();
        const theaterDoc = yield theaterRepository.model.findById(performance.theater.id).exec();
        const theater = theaterDoc.toObject();
        const screenDoc = yield screenRepository.model.findById(performance.screen.id).exec();
        const screen = screenDoc.toObject();
        const filmDoc = yield filmRepository.model.findById(performance.film.id).exec();

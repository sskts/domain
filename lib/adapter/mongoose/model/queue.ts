...
schema.index(
    { group: 1, status: 1, count_tried: 1, run_at: 1 }
);

// 承認系のキューの場合は承認グループごとに処理される
schema.index(
    { group: 1, 'authorization.group': 1, status: 1, count_tried: 1, run_at: 1 }
);

// 通知系のキューの場合は通知グループごとに処理される
schema.index(
    { group: 1, 'notification.group': 1, status: 1, count_tried: 1, run_at: 1 }
);

...
(function (repository) {
    function transaction(connection) {
        return new transaction_1.MongoRepository(connection);
    }
    repository.transaction = transaction;
    function client(connection) {
        return new client_1.MongoRepository(connection);
    }
    repository.client = client;
    function creativeWork(connection) {
        return new creativeWork_1.MongoRepository(connection);
    }
    repository.creativeWork = creativeWork;
    function event(connection) {
        return new event_1.MongoRepository(connection);
    }
    repository.event = event;
    function gmoNotification(connection) {
        return new gmoNotification_1.MongoRepository(connection);
    }
    repository.gmoNotification = gmoNotification;
    function order(connection) {
        return new order_1.MongoRepository(connection);
    }
    repository.order = order;
    function organization(connection) {
        return new organization_1.MongoRepository(connection);
    }
    repository.organization = organization;
    function ownershipInfo(connection) {
        return new ownershipInfo_1.MongoRepository(connection);
    }
    repository.ownershipInfo = ownershipInfo;
    function person(connection) {
        return new person_1.MongoRepository(connection);
    }
    repository.person = person;
    function place(connection) {
        return new place_1.MongoRepository(connection);
    }
    repository.place = place;
    let itemAvailability;
    (function (itemAvailability) {
        function individualScreeningEvent(redisClient) {
            return new individualScreeningEvent_1.MongoRepository(redisClient);
        }
    ...
    function sendGridEvent(connection) {
        return new sendGridEvent_1.MongoRepository(connection);
    }
    repository.sendGridEvent = sendGridEvent;
    function task(connection) {
        return new task_1.MongoRepository(connection);
    }
    repository.task = task;
    function telemetry(connection) {
        return new telemetry_1.MongoRepository(connection);
    }
    repository.telemetry = telemetry;
    function transactionCount(redisClient) {
        return new transactionCount_1.MongoRepository(redisClient);
    }

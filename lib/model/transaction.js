...
class Transaction {
    ...
    getCoaSeatReservationAuthorization() {
        const coaAuthorization = this.authorizations().find((authorization) => (authorization.group === authorizationGroup_1.default.COA_SEAT_RESERVATION));
    ...
    authorizations() {
        ...
        const authorizations = this.events
            .filter((event) => (event.group === transactionEventGroup_1.default.AUTHORIZE))
            .map((event) => event.authorization);
        // 承認解除イベント
        const removedAuthorizationIds = this.events
            .filter((event) => (event.group === transactionEventGroup_1.default.UNAUTHORIZE))
            .map((event) => event.authorization._id.toString());
    ...
    notifications() {
        const notifications = this.events
            .filter((event) => (event.group === transactionEventGroup_1.default.NOTIFICATION_ADD))
            .map((event) => event.notification);
        // メール削除イベント
        const removedNotificationIds = this.events
            .filter((event) => (event.group === transactionEventGroup_1.default.NOTIFICATION_REMOVE))
            .map((event) => event.notification._id.toString());

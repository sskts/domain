...
export function create(args: {
    ...
    // todo validation
    if (validator.isEmpty(args.kgygish_cd)) throw new Error('kgygish_cd required.');
    if (validator.isEmpty(args.yyk_dvc_typ)) throw new Error('yyk_dvc_typ required.');
    if (validator.isEmpty(args.trksh_flg)) throw new Error('trksh_flg required.');
    if (validator.isEmpty(args.kgygish_sstm_zskyyk_no)) throw new Error('kgygish_sstm_zskyyk_no required.');
    if (validator.isEmpty(args.kgygish_usr_zskyyk_no)) throw new Error('kgygish_usr_zskyyk_no required.');
    if (validator.isEmpty(args.jei_dt)) throw new Error('jei_dt required.');
    if (validator.isEmpty(args.kij_ymd)) throw new Error('kij_ymd required.');
    if (validator.isEmpty(args.st_cd)) throw new Error('st_cd required.');
    if (validator.isEmpty(args.scren_cd)) throw new Error('scren_cd required.');
    if (validator.isEmpty(args.skhn_cd)) throw new Error('skhn_cd required.');

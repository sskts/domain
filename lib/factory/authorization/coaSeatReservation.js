...
function create(args) {
    ...
    if (validator.isEmpty(args.coa_theater_code))
        throw new RangeError('coa_theater_code required.');
    if (validator.isEmpty(args.coa_date_jouei))
        throw new RangeError('coa_date_jouei required.');
    if (validator.isEmpty(args.coa_title_code))
        throw new RangeError('coa_title_code required.');
    if (validator.isEmpty(args.coa_title_branch_num))
        throw new RangeError('coa_title_branch_num required.');
    if (validator.isEmpty(args.coa_time_begin))
        throw new RangeError('coa_time_begin required.');
    if (validator.isEmpty(args.coa_screen_code))
        throw new RangeError('coa_screen_code required.');

...
class MongoRepository {
    ...
    pushEvent(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // TODO クライアントの存在確認
            // イベント作成
            const clientEvent = factory.clientEvent.create(Object.assign({}, params, { id: mongoose.Types.ObjectId().toString() }));
            // ドキュメント作成(idが既に存在していればユニーク制約ではじかれる)
            yield this.clientEventModel.findByIdAndUpdate(clientEvent.id, clientEvent, { upsert: true }).exec();
            return clientEvent;
        });
    }
}
